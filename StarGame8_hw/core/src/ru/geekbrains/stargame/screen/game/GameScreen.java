...
public class GameScreen extends Base2DScreen implements ActionListener {
    ...
    private static final float HP_BAR_HEIGHT=0.02f;
    ...
    public void drawHpBar(int hp){
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.setProjectionMatrix(worldToGl);
        if (hp <= 0.25*totalHP)
            shapeRenderer.setColor(0.5f,0,0,1.0f);
        else if (hp<=0.75*totalHP)
            shapeRenderer.setColor(0.5f,0.5f,0,1.0f);
        else shapeRenderer.setColor(0,0.5f,0,1.0f);
//        shapeRenderer.rect(worldBounds.pos.x-HP_BAR_WIDTH/2,worldBounds.getTop()-HP_BAR_HEIGHT,HP_BAR_WIDTH,HP_BAR_HEIGHT);
        shapeRenderer.rect(worldBounds.getLeft(),worldBounds.getBottom(),worldBounds.getWidth()*hp/totalHP,HP_BAR_HEIGHT);
        shapeRenderer.end();
    }
